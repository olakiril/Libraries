classdef YWarp < handle
    ...
    methods(Static)
        function img = apply(img, warp,degrees)
            ...
            for i=0:degrees(1)
                yy = bsxfun(@plus, yy, warp(i+1)*chebyI(yv,i));
            end
            for i=0:degrees(2)
                xx = bsxfun(@plus, xx, warp(i+2+degrees(1))*chebyI(yv,i));
            end
            for i=1:degrees(3)   % zeroth degree omitted
                xx = bsxfun(@plus, xx, warp(i+2+sum(degrees(1:2)))*chebyI(xv,i));
            end
    ...
    methods(Access=private)
        function L = residual(self, img, p)
            ...
            for i=0:self.degrees(1)
                yy = bsxfun(@minus, yy, p(i+1)*chebyI(self.yvec,i));
            end
            for i=0:self.degrees(2)
                xx = bsxfun(@minus, xx, p(i+2+self.degrees(1))*chebyI(self.yvec,i));
            end
            for i=1:self.degrees(3)   % zeroth degree omitted
                xx = bsxfun(@minus, xx, p(i+2+sum(self.degrees(1:2)))*chebyI(self.xvec,i));
            end
