function fail = fastSiteFileVessels(prefix,mag,lens,segment)
...
% set variables when info not there
if nargin<4
    segment = 1;
end
if nargin<3
...
imData = abs(imData - max(imData(:)));
siteFile.meanCh1 = imData;
if ~exist('imageChannel2','var')
    warning('Processing dataset %s with only one channel. You should record both channels', prefix); %#ok<WNTAG>
    siteFile.meanCh2 = zeros(size(siteFile.meanCh1));
else
    imData2 = mean( getData(imageChannel2), 3);
siteFile.meanCh2  = abs(imData2 - max(imData2(:)));
end
...
if ~isStack(tpr) && ~isnan(segment) && segment
    ...
    siteFile.traces = extractTracesFast( imData, siteFile );
    siteFile.Fps = getFramerate(tpr);
