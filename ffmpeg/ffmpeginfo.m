function [info input] = ffmpeginfo(infile)
...
start = regexp(input,'(?<start>, start: \d+\.\d+)?','names');
bitrate = regexp(input,', bitrate: (?<bitrate>(-?\d+)|(N/A))','names');
data = regexp(input,'(?<data>.*)','names');
info.duration = duration.duration;
...
if strcmp(info.duration,'N/A')
    ...
    info.duration = [3600 60 1]*sscanf(info.duration,'%d:%d:%f');
end
if isempty(info.start)
    info.start = [];
else
    info.start = sscanf(info.start,', start: %f');
end
if strcmp(info.bitrate,'N/A')
    info.bitrate = [];
else
...
info.meta = parse_metadata(info.meta,'  Metadata:\n');

% separate chapters & stream info
I0 = regexp(info.data,{'    Chapter','    Program','    Stream'},'start','once');
% parse chapters (if exists)
if isempty(I0{1})
    info.chapters = struct([]);
else
    if ~isempty(I0{2})
        I1 = I0{2}-1;
    elseif ~isempty(I0{3})
    ...
    data = info.data(I0{1}:I1);
    
    info.chapters = regexp(data,['    Chapter #\d+\.(?<number>\d+): '....
        'start (?<start>[\d\.]+), end (?<end>[\d\.]+)\n'...
        '(?<meta>    Metadata:\n.*?)?\n'...
        ],'names');
    
    for m = 1:numel(info.chapters)
        info.chapters(m).start = str2double(info.chapters(m).start);
        info.chapters(m).end = str2double(info.chapters(m).end);
        info.chapters(m).meta = parse_metadata(info.chapters(m).meta,'    Metadata:\n');
    end
end

% parse program (if exists)
if isempty(I0{2})
    info.programs = struct([]);
else
    if ~isempty(I0{3})
        I1 = I0{3}-1;
    else
    ...
    data = info.data(I0{2}:I1);
    
    info.programs = regexp(data,['    Program (?<id>\d+) '....
        '(?<name>[^\n]+)\n'...
        '(?<meta>    Metadata:\n.*?)?\n'...
        ],'names');
    
    for m = 1:numel(info.programs)
        info.chapters(m).meta = parse_metadata(info.chapters(m).meta,'    Metadata:\n');
    end
end

% parse streams
info.streams = regexp(info.data(I0{3}:end),['    Stream #\d+:(?<id>\d+)'...
    '(?<pid>\[0x[\dabcdef]+\])?(?<lang>\(.+?\))?:'...
    ' (?<type>[^:]+): (?<codec>[^\n]+\n)(?<meta>    Metadata:\n.*?\n)?'],...
    'names');
for m = 1:numel(info.streams)
    info.streams(m).id = str2double(info.streams(m).id);
    if isempty(info.streams(m).pid)
        info.streams(m).pid = '';
    else
        info.streams(m).pid = info.streams(m).pid(2:end-1);
    end
    if isempty(info.streams(m).lang)
        info.streams(m).lang = '';
    else
        info.streams(m).lang = info.streams(m).lang(2:end-1);
    end
    info.streams(m).type = lower(info.streams(m).type);
    info.streams(m).codec = parse_codecinfo(info.streams(m).type,info.streams(m).codec);
    info.streams(m).meta = parse_metadata(info.streams(m).meta,'    Metadata:\n');
end
